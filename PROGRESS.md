...
- **加密 K 线补全**：crypto.py `fetch_binance_alt_klines`——山寨币走 Binance USDT 对写 K 线文件 + 3 年收盘历史回填（此前 CoinGecko 只有当前价、山寨币无历史曲线）；K 线文件 161→196；未上市 Binance 的（CC 等）静默跳过保持折线回退
- **Jev 日历重要度**：`jev.score_events` 一次合批给未来 90 天日历事件打 0-1 重要度 → `impSquares` 的 ■ 方块优先用模型分（悬浮标注"判断模型打分"），无密钥回退静态分类分。实测排序合理：FOMC 0.975 > 中期选举 0.93 > ENA 解锁 0.895
- **军费线 USAspending 腿**：`usaspending_dod` 抓 DoD(097) 财年支出同比（财年初进行时口径不足上财年一半时不输出，防假信号）；本机实测直接可达（此前担心的证书问题未复现）；**实测 -6.05% → defense.dod_slow 军费见顶预警真实触发**（规则 225→227）

## 上一断点（2026-09-21 18:20 曼谷）：扩展第 2 批 + 全站标的 K 线（v2026.09.21.8）
